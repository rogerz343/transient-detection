...
layersizes = [sz, 2*sz, 1*sz, 1*sz, 2]
...
# Weights to fourth layer
W4 = tf.Variable(tf.random_normal([layersizes[3], layersizes[4]], stddev=0.1), name='W4')
b4 = tf.Variable(tf.random_normal([layersizes[4]]), name='b4')
...
#hidden3 = tf.nn.dropout(hidden3, 0.8)

# Output of output layer
y_ = tf.nn.softmax(tf.add(tf.matmul(hidden3, W4), b4))
